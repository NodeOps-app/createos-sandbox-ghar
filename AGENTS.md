...
```bash
bun install
pre-commit install                                  # runs those four on every commit
bun run lint && bun run format:check && bun run typecheck && bun run test   # green before you start
```
...
cron (*/5) → src/index.ts scheduled → reconcile.runReconciler → reconcile.runReaper → DO.sweep
...
| `src/reconcile.ts`                | Cron-side self-healing, extracted from `handler.ts` so the webhook hot path and the cron path evolve separately: `runReconciler` (re-drive lost jobs through `admitAndDrive` — the same gate ladder the webhook uses, in both tenancy modes — reap runner-less VMs, delete orphaned registrations, sweep unowned sandboxes) and the age-only `runReaper` backstop. Shares the provisioning/teardown core (`provisionAndRecord`, `destroyAndConfirm`) that stays in `handler.ts`. |
| `src/sandbox.ts`                  | `createRunnerSandbox` (JIT once → createSandbox, looping `CREATEOS_REGIONS` on region-level faults and returning the region that booted the VM) and `launchRunner` as **two steps**, so ownership is recorded in the DO between them; plus idempotent, region-aware `teardownSandbox`. `SandboxDeps.makeClient` is the test seam.                                                                                                                                                |
| `src/shapes.ts`                   | Label ↔ shape mapping and the cached (5 min), floored shape catalog from `GET /v1/shapes`. Label admission is two pure, **silent** functions (the caller has the job id and does the logging): `resolveRequestedLabel` (`none`/`ambiguous`/`one`) and `validateShape`, which is **total** — a bare label short-circuits before touching the catalog. Only `fetchCatalog` hits the network. Callers check policy _before_ fetching a catalog, and even then only lazily.          |
| `src/createos.ts`                 | Builds the CreateOS SDK client (per-region); owns the `CreateosClient` capability interface, `SandboxDeps`, `isFailoverEligible` (the 5xx/connection/timeout failover trigger set), and `regionByName` (teardown's region resolution, which throws on an unknown name rather than guess). Exists to break the `sandbox.ts` ↔ `shapes.ts` import cycle.                                                                                                                           |
| `src/coordinator.ts`              | The `Coordinator` **Durable Object**. ALL state (SQLite): job rows (owning their VM by `runner_name`, and their requested `label`), concurrency counter, pending queue, delivery dedup, `sweep`, and — since the tenant registry — the `tenants`/`projects` tables behind the `admin*` RPC methods. Rows: `pending`→`provisioning`→`running`→`destroying`. Stays passive; never imports `shapes.ts`.                                                                             |
...
| `src/quota.ts`                    | Pure, import-free weighted-minute math (label → shape vCPU → weight; UTC month-key). Import-free on purpose: the DO bills VM lifetimes at teardown but must never import `shapes.ts`, so this is where both Worker and DO can reach the same label→weight parse.                                                                                                                                                                                                                 |
| `src/registry.ts`                 | Tenant/Project persistence over the Coordinator's SQLite (plain functions on `SqlStorage`, no network). Admin-frequency only — never on the webhook hot path.                                                                                                                                                                                                                                                                                                                    |
| `src/admin.ts`                    | Bearer-authed operator API (`/admin/*`) for the tenant registry: installations/upsert/list/status/projects/backfill. 404s — not 401 — when `ADMIN_TOKEN` is unset or the token is wrong, so an unconfigured deployment exposes no probeable surface. `GET /admin/installations` exists because an operator token _cannot_ resolve a community tenant's installation id (see gotchas) — the Worker asks GitHub as the App itself.                                                 |
...
bun run lint        # oxlint
bun run format      # oxfmt — rewrites; the only command that reformats
bun run format:check # oxfmt --check — what the hook and ci.yml run
...
- **`wrangler.toml` `[vars]` ARE the test bindings** — `vitest.config.ts` points the pool at the real config, so a vars-only edit with no `.ts` touched can break the suite. It has: adding the second `CREATEOS_REGIONS` entry gave the fixtures a two-region config, and the fixtures inject one `makeClient` for _every_ region, so the region-looping sandbox sweep saw the same mocked orphan twice and destroyed it twice. Prod was fine (real regions return disjoint lists) — the fixture was not. `miniflare.bindings` therefore pins the vars whose prod value would change fixture behaviour (`TENANCY_MODE`, `CREATEOS_REGIONS`); pin any new one you add, and run the suite after editing `[vars]`.
...
- **The runner lies about its distro: `start-runner.sh` rewrites `/etc/os-release` to claim Ubuntu 22.04.** `actions/python-versions` publishes Ubuntu builds only, and `setup-python` gates on the running distro _before_ it consults the tool cache — on Debian it hard-fails with `The version '3.12' ... was not found for debian 12`, so every `setup-python` job in the org dies. The base-image allowlist is `{nodeops/sandbox:debian, nodeops/sandbox:alpine}`, so we cannot actually _be_ Ubuntu. It claims **22.04, not 24.04**, because the 24.04 python builds link glibc 2.39 and this box has 2.36 — they would download and then refuse to run; the 22.04 builds (glibc 2.35) run clean. The truth stays at `/etc/os-release.debian`. The lie is narrow — apt reads `sources.list`, not `os-release` — but a workflow step that derives an apt repo from the codename now gets `jammy` on a bookworm box and will break. If `nodeops/sandbox:ubuntu` ever reaches the allowlist, delete this hack and switch the base.
- **No language runtime is baked into the template, on purpose.** Go, Node, Bun and Python arrive at job time via `actions/setup-*` and `asdf-vm/actions/install`, because every repo pins its own version (`.tool-versions`, a matrix, `go.mod`) — bake one in and you rebuild the template on every bump and _still_ miss what the repo asked for. What the image owes those installers is the substrate: a C toolchain (cgo, `go test -race`, node-gyp), archivers, `zstd` (actions/cache's codec), and the `/dev` + tool-cache fixups above. `smoke-toolchain` in `ghar-test.yml` is the contract test for exactly that — if you slim the image, it is what tells you what you broke.
- **Changing `RUNNER_LABEL` while jobs are in flight strands rows.** A job's requested label is persisted in `jobs.label` at `onQueued` and re-mapped to a shape at provision time against the _current_ `config.runnerLabel`. Rename the var mid-flight and an old row's label no longer matches — `shapeForLabel` throws rather than slicing a garbage shape out of it, which routes to `markProvisionFailed` (logged, alerted, slot freed). Loud and safe, but expect a provision-failure alert per row queued before the rename.
...
- **A region name in `CREATEOS_REGIONS` is persisted on every job row and read back at teardown — renaming strands rows, exactly like `RUNNER_LABEL`.** Worse than the label case, though: teardown of a stranded row throws on the unknown name (see `regionByName`), so the row parks in `destroying` and the reaper retries + alerts forever instead of leaking the VM — loud, but it never resolves until the old name is re-added. Renaming a region is a migration, not an edit: keep the old name in the list until its rows have drained. The same hazard shapes the rollback rule: Worker rollback reverts code only, env vars stay — so a rollback to pre-failover code while rows name a non-primary region leaves those rows torn down against the wrong control plane (404 → falsely confirmed destroyed → leaked VM). Disable `CREATEOS_REGIONS` and let non-primary rows drain BEFORE rolling back past this feature.
- **Teardown must dial the region that booted the VM, and failover must never trigger on a 4xx.** Another region's control plane has never heard of the sandbox id: its 404 reads as "already destroyed" (teardownSandbox's idempotency) and the VM leaks silently. Every teardown path therefore resolves the row's persisted `region` — NULL (pre-region rows) means the primary. Symmetrically, the provision failover loop retries the next region ONLY on region-level faults (5xx — capacity exhaustion surfaces as a bare 503 — connection, timeout): a 4xx is a request defect that fails identically in every region, and retrying one doubles the latency of a doomed job while a real regional outage queues behind it. When _every_ region refuses that way, one extra attempt goes to the **primary only**, after `RETRY_ALL_REGIONS_MS` — deliberately one attempt at one region, because re-running the whole ladder immediately doubles the create load on a control plane that is already shedding it, and overload is the failure being recovered from. Spaced-out retries are the Coordinator's job (`#requeueForRetry`, a cron tick apart).
...
- **The orphaned-runner sweep must never key on the `cos-` prefix alone.** A JIT registration exists from the moment it is minted, but its VM needs ~30s to boot — so for that whole window a perfectly healthy runner is `offline` and not busy, i.e. _indistinguishable from an orphan by name and status_. Only `Coordinator.liveJobIds()` tells them apart: `onQueued` inserts the job row **before** `createRunnerSandbox` mints the JIT, so a booting runner always has a row and a true orphan never does. Drop that check and the sweeper eats live boots. Equally: mint (`runnerNameFor`) and parse (`jobIdFromRunnerName`) live together in `sandbox.ts` on purpose — let them drift and the sweeper either strands our orphans or deletes another system's runners (the org also hosts ARC runners).
- **The GitHub runner list is a shared, capped resource.** `#getPaged` stops at `MAX_PAGES` (2000 runners). The reconciler's liveness oracle tests for _absence_, so a silently truncated list would read a live runner as gone and destroy its VM mid-job — which is why `listRunners` throws on truncation instead of warning. Orphaned registrations accumulating toward that cap is therefore a correctness risk, not just clutter.
- **A provision that fails after `createSandbox` returns must never delete its row.** The VM exists but its runner never launched, so it will _never_ self-delete — and the DO row is the only record that it exists. `markProvisionFailed` therefore parks a VM-owning row in `destroying` (carrying the `sandboxId`, which the row may not have learned yet if `recordSandboxCreated` is what failed) instead of dropping it, so a compensating destroy that also fails is retried by the reaper rather than leaking forever, invisibly. `destroying` does not count against the cap, so the slot still frees at once.
- **A provision that fails BEFORE `createSandbox` returns must not delete its row either — it goes back on the pending queue.** The job is still `queued` at GitHub and its `queued` webhook has already fired, so dropping the row leaves the reconciler's recovery scan as the only way back — and that scan is `O(installed repos)` under a subrequest budget, so on a 300-repo installation it covers about a third of the org per tick. One transient CreateOS 5xx therefore cost a job ~20 minutes of sitting `queued` (measured 2026-08-10: three jobs stranded 19m43s while every dashboard looked healthy). `markProvisionFailed` re-queues the row instead (`#requeueForRetry`), which the next tick's `sweep` → `#drainPending` picks up at zero GitHub cost. Two invariants hold it together: `provision_started_at` is reset to NULL (else `ROW_AGE` judges the retry by the failed attempt's clock and reaps it mid-boot), and `#dequeuePending` is called with the parked job EXCLUDED (else the row, now the oldest `pending`, instantly re-promotes itself into the slot its own failure freed and retries inside the same invocation). Bounded by `MAX_PROVISION_ATTEMPTS`, and only for failures that could plausibly differ next time — `isPermanentProvisionFailure` (a CreateOS 4xx other than 429) still drops the row at once, because an unknown shape or a rejected key fails identically forever and would otherwise alert once per attempt.
- **`SANDBOX_NAME_PREFIX` length is a safety property, not cosmetics.** The orphaned-sandbox sweep re-derives a VM's owner from its _name_, and `clampSandboxName` truncates at 22 chars — irreversibly and plausibly. With prefix `gha-ci-nodeops-app`, job 86749416515 mints `gha-ci-nodeops-app-86749416515`, which clamps to `gha-ci-nodeops-app-867` — a name that parses cleanly as job **867** and even round-trips back to itself. Nothing in the name reveals the lie, so a sweep acting on it would find no row for job 867 and **destroy the VM while job 86749416515 was still running on it**. No parser can recover the lost digits, so `sandboxNamesAreSweepable` refuses ownership wholesale for any prefix that can truncate (it warns and disables the sweep; leaked VMs then stop being reclaimed). The deployed prefix `gha-ci` is 6 chars and never clamps. Keep it that way.
- **VM names are per-JOB, not per-attempt, so the sweep's ownership oracle must be per-VM.** `gha-ci-<jobId>` deliberately carries no attempt token — it has to fit `clampSandboxName`'s 22 chars, and a 12-digit job id leaves no room for one. So several live VMs can share a name: a `createSandbox` that 5xx'd _after_ the control plane created the VM leaks it (measured 2026-08-10 — six leaked VMs across two regions, one per region per failed provision), and the job's retry creates another under the same name. An oracle keyed on job id alone reads both as "this job is live" and shields the leak for the entire life of the job, burning capacity during exactly the burst that produced it. `Coordinator.liveVmOwnership` therefore pairs each row with the `sandbox_id` it owns, and the sweep destroys a VM whose name is ours when the row names a **different** VM. The null case is load-bearing in the other direction: a row between `createSandbox` returning and `recordSandboxCreated` persisting the id owns a live VM it cannot yet name, so `sandbox_id IS NULL` must read as "spare it", never as "owns something else". Do not "fix" this by lengthening the name instead — the 22-char cap is hard, and per-VM ownership makes name uniqueness unnecessary.
- **The orphaned-sandbox sweep must not be gated on GitHub.** Reconciler steps A/B/C all read GitHub; step D does not, on purpose. The VMs it reclaims leak because a _DO_ call or a destroy failed, which has nothing to do with GitHub — and a leaked VM burns capacity for as long as it lives. A `return` in step B's catch (which is what it used to do) silently took the sweep down with the GitHub outage.
- **Never age a row off `created_at`.** Both destructive age tests (`reapUnregistered`'s grace, `sweep`'s max-age) must use `ROW_AGE` = `COALESCE(provision_started_at, created_at)`. `created_at` is queue-entry time, which a `pending` row keeps while it waits out the cap — so aging off it makes a promoted job _already stale the instant it starts booting_: it gets reaped mid-boot (its runner cannot be online yet), the reconciler re-drives it off GitHub's still-`queued` view, and it is reaped again. A livelock that appears only under the backlog the queue exists to absorb, which is why it survived the first round of tests. `created_at` remains the FIFO key and the pending-expiry key — those two are correct.
- **A valid webhook signature says nothing about _whose_ VM the sender may touch.** The App is public, so every installation's `workflow_job` events are signed with the same secret — all genuine, all mutually untrusted. `completed` and `in_progress` key on `runner_name`, which the sender chooses: a tenant can name its own self-hosted runner `cos-<someone-elses-job>-aa`, complete a job on it, and GitHub will send us a perfectly valid `completed` naming another tenant's live runner. Unscoped, that destroyed that tenant's VM mid-job. Both lookups therefore carry the sending `installation.id` and go through `Coordinator#ownedBy`, which fails closed on both edges (no installation reaches only NULL-`tenant_id` rows; an installation reaches only its own). Any new webhook-reachable DO method that resolves a row by attacker-supplied identity must do the same — the signature check is not the authorization check.
- **An operator token can never resolve a community tenant's installation id — only the Worker can.** `gh api orgs/<org>/installations` needs org-admin on the _applicant's_ org, which we by definition do not have, and it fails with a **404** that is indistinguishable from "they never installed the App" — so the obvious command lies in the one direction that makes you reject a valid applicant. The App-JWT alternative (`GET /orgs/<org>/installation`, `GET /app/installations`) works for any org regardless of rights, because the App can always see its own installations — but it needs `GITHUB_APP_PRIVATE_KEY`, which exists solely as a `wrangler secret` with no operator-side copy, by design. `GET /admin/installations` closes that gap: the Worker holds the key and asks GitHub as the App. Do not "simplify" it back to a `gh` call, and do not add the private key to an operator machine to avoid the round trip.
- **`TENANCY_MODE=multi` requires the Plan 3 cutover runbook** — flipping it without a seeded tenant registry refuses every job (fails closed, loudly).
- **A DO RPC that throws across the real external stub poisons the stub for the rest of the test.** Cloudflare docs say a stub is unusable after one of its calls rejects, and empirically an exception that crosses the real stub (not `runInDurableObject`) corrupts `@cloudflare/vitest-pool-workers@0.8.71`'s isolated-storage bookkeeping badly enough to crash the whole test run — `test/integration/registry.test.ts` works around it with `runInDurableObject`, and `admin.ts`'s routes pre-check with a plain read (e.g. `adminGetTenant`) instead of letting a DO guard throw (e.g. `adminAddProjects` on an unknown tenant) cross the stub as a 404. Follow the same pattern for any new admin route: read first, throw never.

## Conventions

- **bun only** — never npm/npx/node. Pin exact versions (`bun add -E`).
- **Use zod** for runtime validation/parsing
- **A push to `main` is a production deploy.** The repo is connected to **Cloudflare Workers Builds**: Cloudflare builds and deploys from `main` itself — there is no deploy step in `.github/workflows/`, and nobody runs `wrangler deploy` by hand. Merging _is_ shipping, so a change lands in prod the moment it lands on `main`. `bun run deploy` remains as an escape hatch for a hotfix you cannot push, but it deploys your **working tree, not `main`** — it will happily ship uncommitted local edits, and the next push overwrites it. Reach for it only when you mean it. **`ci.yml` does not gate any of this**: Workers Builds and GitHub Actions are separate systems, so a commit that fails lint/typecheck/test still deploys. Run the checks _before_ you merge, not after.
- **Always be ready to roll back before pushing to prod.** Since a push _is_ the deploy, this applies to every merge: capture the current live version (`bunx wrangler@latest deployments list` → note the active version id), know the rollback command (`bunx wrangler@latest rollback [version-id]`), and confirm the change is roll-back-safe. Worker rollback reverts **code only** — a DO SQLite migration does **not** auto-revert, so every migration must be forward-compatible (additive `ALTER TABLE ADD COLUMN`; old code ignores new columns) and never destructive in a single deploy. After deploying, watch the smoke run; if it fails, roll back first and diagnose second.
...
- Self-documenting code over comments; comment the _why_, not the _what_.
...
1. `bun run lint && bun run format:check && bun run typecheck && bun run test` — all green, no new warnings.
