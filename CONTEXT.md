...
- **Provisioning policy** — **single-mode term.** Configurable switch deciding which Jobs get a Sandbox: `org-wide` (any repo, default), `repo-allowlist` (only listed repos), or `fork-gated` (skip fork-PR jobs). In `TENANCY_MODE=multi` this env policy is bypassed — **the Tenant registry IS the policy** (Tenant approved + Project on its allowlist, see Tenant admission). Under `org-wide`, fork-PR safety rests solely on VM isolation + ephemerality. The policy is a **cost** control — which repos the Controller spends a VM on — not a GitHub-side execution boundary: the JIT Runner registers into the Tenant's runner group (NodeOps: the org-wide Default group `1`), and which repos may schedule onto a Runner is that *group's* GitHub policy, not the allowlist.
