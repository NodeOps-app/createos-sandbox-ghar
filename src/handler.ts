...
export async function provisionAndRecord(
  ...
  try {
    ...
    // D15: every VM gets a per-VM egress quota, regardless of tenant — including
    // allow-all tenants (NodeOps) and single mode. The control plane REJECTS the
    // quota at create (400 "not settable at create; use …/bandwidth/recharge"),
    // so we top up here — AFTER ownership is recorded (a Worker crash mid-call
    // leaves a tracked, reaper-reclaimed VM, not an orphan) and BEFORE launch.
    // Recharge is ADDITIVE and a fresh VM already carries the account default
    // (5 GiB, probed 2026-07-24), so we add only the delta to the configured cap.
