...
| `src/admin.ts` | Bearer-authed operator API (`/admin/*`) for the tenant registry: upsert/list/status/projects/backfill. 404s — not 401 — when `ADMIN_TOKEN` is unset or the token is wrong, so an unconfigured deployment exposes no probeable surface. |
