...
- `src/shapes.ts` — label ↔ shape mapping and the cached (5 min) + floored shape catalog from `GET /v1/shapes`. Label admission is `selectLabel(labels, config, catalog): LabelSelection` — pure and silent (never logs; the caller has the job id and does the logging) against a `Catalog` (`{ok: true, usable: Set<string>} | {ok: false}`), which only `fetchCatalog(config, deps)` produces by hitting the network (converting a throw into `{ok: false}` rather than propagating). `createosLabels`, `shapeForLabel`, `selectLabel` never touch the network; callers fetch a `Catalog` lazily, only when a job might actually need one.
...
- `src/github/{jwt,auth,client}.ts` — zero-dep GitHub App auth: RS256 JWT (Web Crypto) → installation-token cache → `GitHubClient` (`generateJitConfig(runnerName, label)`, `isForkJob(repoFullName, runId)`, `listQueuedJobs(): Promise<QueuedJob[]>`). `listQueuedJobs` is dumb transport — no arguments, no shape policy, no `pickLabel` — it returns every job GitHub reports `queued` with its raw labels; label admission (`selectLabel`/`fetchCatalog`, see `src/shapes.ts`) is the caller's job, not this client's. API base is `config.githubApiUrl` (`GITHUB_API_URL`, default api.github.com).
